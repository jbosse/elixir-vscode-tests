    ...
    docSelector,
    {
      async provideCodeLenses(document) {
        ...
        let lenses = describes.map((describe) => {
          return new vscode.CodeLens(describe.range, { command: "elixir.lens.test.run", title: title, arguments: [kind, describe.line, document] });
        });
        return lenses;
      }
    }
  ...
  return vscode.languages.registerCodeLensProvider(
    docSelector,
    {
      async provideCodeLenses(document) {
        ...
        return [new vscode.CodeLens(topOfDocument, { command: "elixir.lens.test.run", title: "Run tests", arguments: ["file", 0, document] })];
      }
    }
  );
};
