...
suite('Lens Providers Test Suite', () => {
  ...
  test("fileLensProvider", async () => {
    ...
    assertDefinition("elixir.lens.test.run", "Run tests", "file", 0, fakeDocument, lenses[0]);
  });
  ...
  test("describeLensProvider", async () => {
    ...
    assertDefinition("elixir.lens.test.run", "Run tests", "describe", 0, fakeTestDocument, lenses[0]);
    assertRange([3, 3, 3, 38], lenses[1].location);
    assertDefinition("elixir.lens.test.run", "Run tests", "describe", 3, fakeTestDocument, lenses[1]);
  });
  ...
  test("testLensProvider", async () => {
    ...
    assertDefinition("elixir.lens.test.run", "Run test", "test", 1, fakeTestDocument, lenses[0]);
    assertRange([4, 3, 4, 30], lenses[1].location);
    assertDefinition("elixir.lens.test.run", "Run test", "test", 4, fakeTestDocument, lenses[1]);
  });
  ...
  test("itLensProvider", async () => {
    ...
    assertDefinition("elixir.lens.test.run", "Run test", "it", 2, fakeTestDocument, lenses[0]);
    assertRange([5, 3, 5, 26], lenses[1].location);
    assertDefinition("elixir.lens.test.run", "Run test", "it", 5, fakeTestDocument, lenses[1]);
  });
});
